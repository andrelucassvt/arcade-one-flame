...
4. **Cubit de controles** — `lib/title/view/title_page.dart` → `TitleView.initState`
   Cria `TitleControlModeCubit` com o `StorageService` disponivel acima da tela e chama `init()` para restaurar o modo salvo em `title_control_mode`.
5. **Home** — `lib/title/view/title_page.dart` → `TitleView.build`
   Le `AppLocaleCubit` via `context.select`, expoe `TitleControlModeCubit` com `BlocProvider.value`, reage ao modo atual com `BlocBuilder` e monta fundo espacial (`TitleBackdrop`), top bar (`TitleTopBar`) e conteudo principal (`TitleMainContent`). O conteudo fica dentro de `SingleChildScrollView` para evitar overflow em telas baixas.
...
10. **Acao do usuario** — `lib/title/content/title_start_button.dart` → `TitleStartButton`
   Exibe um `ElevatedButton.icon` com `l10n.titleButtonStart` e recebe o `GameControlMode` selecionado.
11. **Navegacao** — `lib/title/content/title_start_button.dart` → `onPressed`
   Ao tocar no botao, faz `Navigator.of(context).pushReplacement(GamePage.route(controlMode: controlMode))`.
12. **Destino** — `lib/game/view/game_page.dart` → `GamePage.route`
...
| Testes | `test/title/view/title_page_test.dart` | Cobre renderizacao, troca de idioma, mute, navegacao, persistencia e restauracao do modo de controle da tela de titulo. |
...
- A feature Title agora tem um Cubit proprio para a preferencia de controles; os estados globais consumidos continuam sendo `AppLocaleCubit` e `AudioCubit`.
- O idioma escolhido e persistido via `StorageService` e restaurado automaticamente no proximo lancamento.
- O modo de controle escolhido tambem e persistido via `StorageService` e restaurado automaticamente quando a Title abre novamente.
